...
if ($LocalRepo) {
	Write-Host "Creating Nuget Package for module: 'BeeperPS' at '$(Get-Location)"
	Publish-PSFModule -Path "$($publishDir)\BeeperPS" -DestinationPath . -SkipDependenciesCheck
	return
}

# Publish to Gallery
Write-Host "Publishing the BeeperPS module to $($Repository)"
$param = @{
	Path                  = "$($publishDir)\BeeperPS"
